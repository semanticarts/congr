...
def spacetime_hash(input_string):
    return hashlib.sha256(str(input_string).encode()).hexdigest()[:10]
...
def generate_node(name, hash):
    return URIRef(congr3 + "_Content_" + quote(name, safe='') + "_" + hash)

def handle_directory(root, g, starting_dir_node):
    dir_hash = spacetime_hash(root + str(os.path.getctime(root)))
    dir_node = generate_node(os.path.basename(root), dir_hash)
...
def handle_file(file_path, filename, g, create_fingerprints, dir_node=None):
    file_location_hash = spacetime_hash(file_path + str(os.path.getctime(file_path)))
    file_node = generate_node(filename, file_location_hash)
...
def generate_file_metadata(starting_dir_path, include_files, create_fingerprints, output_file='congr-output.ttl'):
    ...
    starting_dir_node = generate_node(os.path.basename(starting_dir_path), starting_dir_hash)
    ...
    for root, dirs, files in os.walk(starting_dir_path):
        ...
        if root != starting_dir_path:
            dir_node = handle_directory(root, g, starting_dir_node)
        
        for filename in files:
            file_path = os.path.join(root, filename)
            if os.path.isfile(file_path) and include_files:
                handle_file(file_path, filename, g, create_fingerprints, dir_node)
